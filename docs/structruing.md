...
建议使用 Python `3.12` 下的 `pytest` 进行测试，第一版至少覆盖以下场景。
...
- 单关键词查询关系可返回命中结果
- `top_k` 能正确截断结果数量
- `depth=0/1/2/3` 在图查询中行为正确
- 空 `keywords` 输入时返回空结果或明确异常

## 13.4 同步测试

- 写入后未达到 `sync_interval_seconds` 时不立即落盘
- 写入后超过同步间隔时自动触发落盘
- 调用 `flush()` 时立即同步脏数据
- 多次写入合并为一次批量同步
- 程序退出前同步逻辑可正常执行
...
- 在 Python `3.12` 下完整运行测试集
- package 安装后可正常导入 `EndBrain`
- 核心 API 在示例调用下行为符合预期
...
- 测试：使用 `pytest` 规划初始化、写入、查询、同步、恢复、淘汰、一致性测试
